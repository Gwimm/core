...
int main(int argc, char **argv) {
    ...
    while ((arg = getopt(argc, argv, "i:o:m:c:")) != -1) {
	    switch(arg) {
	    	case 'i':
	    		borders[2] = strtoul(optarg, NULL, 10);
	    		break;
	    	case 'o':
	    		borders[0] = strtoul(optarg, NULL, 10);
	    		break;
	    	case 'm':
	    		borders[1] = strtoul(optarg, NULL, 10);
	    		break;
	    	case 'c':
...
static void usage(void) {
    die("usage: [-c base color] [-i inner border size] [-o outer border size] [-m middle border size] wid [...]\n");
}
...
static void set_border(long color, xcb_window_t win) {
	uint32_t values[1];
	short w, h, b, x, y, z;

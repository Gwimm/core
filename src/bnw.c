...
int main(int argc, char *argv[]) {
    ...
    while (optind < argc)
        set_border(strtoul(argv[optind++], NULL, 16));
...
static void usage(void) {
    die("chwbn - give \"n\" borders to programs\n"
        "chwbn [-b \"n\"] [-c color]\n");
}

static void get_rectangles(xcb_rectangle_t rectangles[], \
        short offset, short cbw, short bw, short h, short w) {
  /* outside in */
...
static void set_border(xcb_window_t win) {
    uint32_t values[1];
    short w, h, bw, cbw;
